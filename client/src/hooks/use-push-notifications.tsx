...
// Interface para registrar um dispositivo no backend
...
export function PushNotificationsProvider({
  ...
  // Implementação direta do hook para evitar chamadas indiretas que podem causar problemas
  ...
  const subscribe = async () => {
    try {
      ...
      const currentDeviceType = isIOS() ? "ios" : "web";
  ...
  const unsubscribe = async () => {
    try {
        ...
        try {
          ...
          if (iosDevices.length > 0) {
            ...
            toast({
              ...
              description: "Você não receberá mais notificações neste dispositivo iOS.",
            });
          } else {
            toast({
              title: "Nenhum dispositivo encontrado",
              description: "Não encontramos registros de dispositivos iOS para desativar.",
              variant: "destructive",
            });
          }
        } catch (error) {
          console.error("Erro ao desativar notificações iOS:", error);
          toast({
            title: "Erro ao desativar notificações",
            description: "Ocorreu um erro ao desativar as notificações no iOS.",
            variant: "destructive",
          });
        }
      ...
      if (!subscription) {
        ...
        toast({
          title: "Não inscrito",
          description: "Você não está inscrito para notificações.",
          variant: "destructive",
        });
        setIsPending(false);
        return;
  ...
  const testNotification = async (options?: NotificationTestOptions) => {
    try {
      ...
      if (payload.platform === undefined && isIOS()) {

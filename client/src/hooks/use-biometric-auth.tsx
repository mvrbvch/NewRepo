...
export function useBiometricAuth() {
  ...
  const registerVerifyMutation = useMutation({
    mutationFn: async ({ credential, deviceName }: { credential: any, deviceName: string }) => {
      const response = await apiRequest('POST', '/api/webauthn/register-verify', {
        credential,
        deviceName
      });
      return await response.json();
  ...
  const loginOptionsMutation = useMutation({
    mutationFn: async (username: string) => {
      const response = await apiRequest('POST', '/api/webauthn/login-options', { username });
      return await response.json();
    }
  });
  ...
  const loginVerifyMutation = useMutation({
    mutationFn: async ({ credential, userId }: { credential: any, userId: number }) => {
  ...
  const removeCredentialMutation = useMutation({
    mutationFn: async (credentialId: string) => {
      const response = await apiRequest('DELETE', `/api/webauthn/credentials/${credentialId}`);
      return await response.json();
    }
  });

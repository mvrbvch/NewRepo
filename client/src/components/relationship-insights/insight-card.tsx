...
export function InsightCard({ insight, isPartner = false }: InsightCardProps) {
  ...
  const typeConfig =
    insightTypeConfig[insight.insightType.toLowerCase()] ||
    insightTypeConfig.general;
  ...
  return (
    <Card
      className={cn(
        ...
        !isRead ? "border-[#F27474]/30 shadow-sm" : "border-gray-200",
      )}
    >
      {!isRead && (
      ...
      <CardHeader className="pb-1 pt-3 px-3 relative">
        <div className="flex justify-between items-start">
          <div className="flex flex-wrap items-center gap-1">
            ...
            {insight.sentiment && (
              <Badge
                variant="secondary"
                className={cn(
                  sentimentColors[insight.sentiment.toLowerCase()],
                  "text-xs font-normal",
                )}
              >
            ...
            {!isRead && (
              <Badge
                variant="secondary"
                className="bg-[#F27474]/10 text-[#F27474] text-xs font-normal"
              >
                Novo
              </Badge>
            )}
          ...
          {insight.score && (
            <Badge
              variant="outline"
              className="flex items-center gap-1 bg-[#F27474]/5 text-[#F27474] text-xs font-normal border-[#F27474]/20"
            >
        ...
        <CardTitle className="text-lg mt-2 font-medium text-[#F27474]">
          {insight.title}
        </CardTitle>
              ...
              <ul className="space-y-1.5">
                {(insight.actions as any[]).map((action, index: number) => {
                  ...
                  return (
                    <li
                      key={index}
                      className="flex items-start gap-1.5 text-xs bg-white p-2 rounded border border-gray-100"
                    >

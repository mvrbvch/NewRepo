...
export function TipList({ type }: TipListProps) {
  ...
  const CategoryBadge = ({ category }: { category: TipCategory }) => {
    const colorMap = {
      [TipCategory.COMMUNICATION]: "bg-blue-100 text-blue-800 hover:bg-blue-100/80",
      [TipCategory.QUALITY_TIME]: "bg-purple-100 text-purple-800 hover:bg-purple-100/80",
      [TipCategory.CONFLICT_RESOLUTION]: "bg-orange-100 text-orange-800 hover:bg-orange-100/80",
      [TipCategory.RELATIONSHIP_GROWTH]: "bg-green-100 text-green-800 hover:bg-green-100/80",
      [TipCategory.SHARED_GOALS]: "bg-teal-100 text-teal-800 hover:bg-teal-100/80",
      [TipCategory.DAILY_HABITS]: "bg-amber-100 text-amber-800 hover:bg-amber-100/80"
    };
    
    const className = colorMap[category] || "bg-gray-100 text-gray-800 hover:bg-gray-100/80";
      ...
      <div className="w-full flex justify-center items-center py-6">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-2 text-sm text-gray-500">
            {generateTip.isPending ? "Gerando nova dica..." : "Carregando dicas..."}
          </p>
        </div>
      </div>
    );
    ...
    <div className="w-full">
      {type === "all" && (
        <div className="flex flex-wrap justify-between items-center gap-2 mb-6">
          <h2 className="text-xl font-semibold text-gray-800">Dicas de Relacionamento</h2>
          <div className="flex gap-2">
            <Button onClick={() => handleGenerateTip()} variant="default" size="sm">
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="daily_habits">Hábitos Diários</TabsTrigger>
        </TabsList>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {tips.filter(tip => tip.category === category).length === 0 && (
                <div className="col-span-2 text-center py-10">
                  <p className="text-gray-500 mb-4">Nenhuma dica nesta categoria</p>
                  <Button onClick={() => handleGenerateTip(category)} variant="outline">
...
function TipCard({ tip, onFavorite, onUnfavorite, onDelete }: TipCardProps) {
  const createdAt = typeof tip.createdAt === 'string'
    ? new Date(tip.createdAt)
    : tip.createdAt;

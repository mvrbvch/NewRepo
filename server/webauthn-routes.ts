...
export function registerWebAuthnRoutes(app: Express) {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const username = req.user?.email as string;
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        console.log("Verificando registro WebAuthn...", req.body);
        const userId = req.user?.id as number;
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log("recebendo opções de autenticação WebAuthn...", req.body);
        if (!username) {
          return res
            .status(400)

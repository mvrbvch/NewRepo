...
export function registerWebAuthnRoutes(app: Express) {
  ...
  app.post('/api/webauthn/register-verify', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!credential) {
        return res.status(400).json({ message: "Dados de credencial faltando" });
      }
      ...
      if (!verification.verified) {
        return res.status(400).json({ message: "Falha na verificação da credencial" });
      }
  ...
  app.post('/api/webauthn/login-options', async (req: Request, res: Response) => {
    try {
      ...
      if (!options) {
        return res.status(400).json({ 
          message: "Falha ao gerar opções de autenticação. Verifique se o usuário existe e possui credenciais registradas." 
        });
      }
  ...
  app.post('/api/webauthn/login-verify', async (req: Request, res: Response) => {
    try {
      ...
      const verification = await verifyWebAuthnAuthentication(userId, credential);
      
      if (!verification.verified || !verification.user) {
        return res.status(401).json({ message: "Falha na autenticação biométrica" });
      }
      
      // Login do usuário
      req.login(verification.user, (err) => {
        if (err) {
          console.error('Erro ao fazer login após autenticação biométrica:', err);
  ...
  app.delete('/api/webauthn/credentials/:id', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!credentialId) {
        return res.status(400).json({ message: "ID da credencial é obrigatório" });
      }
      ...
      if (result) {
        ...
        res.status(404).json({ message: "Credencial não encontrada ou já removida" });
      }
    } catch (error) {

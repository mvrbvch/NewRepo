import { InsightGenerationResult, InsightType, TaskDistributionData } from "./relationshipInsightsService";
...
export class PerplexityService {
  ...
  public async generateTaskDistributionInsight(
    ...
    if (!this.isConfigured()) {
      console.warn("API Perplexity não configurada (PERPLEXITY_API_KEY não definida)");
      return null;
    }
    ...
    try {
      ...
      // Construir o prompt
      const systemMessage = "Você é um assistente especializado em insights de relacionamento com base em dados.";
      const userMessage = `
...
`;
      ...
      if (cleanedText.includes('```')) {
        ...
        if (match && match[1]) {
          ...
          console.warn("Não foi possível extrair o conteúdo JSON dos delimitadores de código markdown");
        }
      }

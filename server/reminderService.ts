...
export class ReminderService {
  ...
  private async processEventReminders(): Promise<void> {
    ...
    if (pendingReminders.length > 0) {
      console.log(
        `Processando ${pendingReminders.length} lembretes de eventos pendentes`
      );
  ...
  private async processTaskReminders(): Promise<void> {
    ...
    if (pendingReminders.length > 0) {
      console.log(
        `Processando ${pendingReminders.length} lembretes de tarefas pendentes`
      );
  ...
  private async sendEventReminder(reminder: EventReminder): Promise<boolean> {
    try {
      ...
      const formattedDate = format(event.date, "d 'de' MMMM yyyy", {
        locale: ptBR,
      });
      ...
      if (reminder.reminderType === "email") {
        ...
        sent = successCount > 0;
        console.log(
          `Notificação push enviada para ${successCount} dispositivo(s) do usuário ${user.id}`
        );
      }

...
import { InsightType, RelationshipInsightsService } from "./relationshipInsightsService";
import { z } from "zod";

/**
 * Configura as rotas para o sistema de insights de relacionamento
 */
export function setupRelationshipInsightsRoutes(app: express.Express, storage: IStorage) {
  ...
  app.get("/api/relationship-insights", async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ error: "Erro ao obter insights de relacionamento" });
    }
  });
  ...
  app.get("/api/relationship-insights/partner", async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ error: "Erro ao obter insights do casal" });
    }
  });
  ...
  app.post("/api/relationship-insights/:id/read", async (req: Request, res: Response) => {
    try {
      ...
      const updatedInsight = await storage.markInsightAsRead(insightId, isUser);
      
      return res.json(updatedInsight);
    } catch (error) {
      console.error("Erro ao marcar insight como lido:", error);
      return res.status(500).json({ error: "Erro ao marcar insight como lido" });
    }
  });
  ...
  app.post("/api/relationship-insights/generate", async (req: Request, res: Response) => {
    try {
      ...
      if (!parseResult.success) {
        return res.status(400).json({ error: "Dados inválidos", details: parseResult.error });
      }
      ...
      const generateMethod = (insightsService as any).generateInsightsForCouple;

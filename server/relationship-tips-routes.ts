...
import { RelationshipTipsService, TipCategory } from './relationshipTipsService';
...
router.post('/api/tips/generate', async (req, res) => {
  try {
    const { userId, partnerId, category, customPrompt, contextData } = req.body;
    
    if (!userId || !partnerId) {
      return res.status(400).json({ error: 'Usuário e parceiro são obrigatórios' });
    }
...
router.get('/api/tips/user/:userId', async (req, res) => {
  try {
...
router.post('/api/tips/:id/favorite', async (req, res) => {
  try {
    ...
    if (!success) {
      return res.status(404).json({ error: 'Dica não encontrada ou erro ao salvar' });
    }
...
router.post('/api/tips/:id/unfavorite', async (req, res) => {
  try {
    ...
    if (!success) {
      return res.status(404).json({ error: 'Dica não encontrada ou erro ao remover' });
    }
...
router.get('/api/tips/favorites/:userId', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar dicas de relacionamento favoritas' });
  }
});
...
router.delete('/api/tips/:id', async (req, res) => {
  try {
    ...
    if (!success) {
      return res.status(404).json({ error: 'Dica não encontrada ou erro ao excluir' });
    }
